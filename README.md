...
사용자가 제공한 포스터와 1~5차 캐스팅 스케줄 이미지를 직접 판독한 데이터입니다. 일정은 2026-08-16~2026-11-15이며, 공개 차수별 25 / 21 / 15 / 29 / 30회, 총 120회입니다. 배역 6개와 배우 17명을 모두 저장하고 선택 필터는 엘리자벳·토드·루케니만 노출합니다. 이미지는 한국 시간으로 읽었으며 Preview·첫공·막공 표시는 `schedule_notes`에 보관합니다.

- `data/elisabeth-2026-transcription.txt`: 이미지 행 순서의 판독 원문. 시간은 24시간제로 변환.
- `data/elisabeth-2026.json`: 전체 배역과 공개 차수를 포함한 구조화 데이터.
- `public/posters/elisabeth-2026.png`: 사용자가 제공한 포스터 원본.
- `supabase/imports/elisabeth-2026.sql`: 초기 테이블이 존재하는 DB의 SQL Editor에서 전체 실행. 필요한 추가 컬럼·뷰와 엘리자벳 공연·120회차를 한 트랜잭션으로 저장합니다. 재실행 시 동일 공연/회차는 제공 이미지 내용으로 갱신하며 다른 공연은 삭제하지 않습니다. 마지막 결과는 차수별 25, 21, 15, 29, 30이어야 합니다.

Supabase 환경값이 설정된 화면은 DB만 조회합니다. 미설정 시에는 가상 공연 대신 첨부 스케줄 기반 로컬 미리보기를 표시하며, DB 저장 완료로 표시하지 않습니다. 포스터 경로는 웹앱에서 제공되므로 Table Editor의 직접 이미지 미리보기 URL이 아닙니다.

## 회원 로그인 및 시간표 제출

공연 목록과 캐스팅 조회는 비로그인 상태에서도 가능합니다. 관심 회차와 시간표 제출은 로그인해야 하며, 관심 회차는 계정별 DB에 저장합니다. 일반 회원은 `/submit`에서 최대 5장(장당 8MB)의 시간표를 검수 대기로 제출합니다. 본인은 자신의 제출 내역만, 관리자는 전체 제보 내역과 원본을 볼 수 있습니다. 일반 회원에게 공연 직접 등록·게시 권한은 부여하지 않습니다.

`supabase/migrations/202609220005_member_submissions.sql`을 SQL Editor에서 실행해야 제보함과 비공개 이미지 저장소가 활성화됩니다. 현재 제보함은 접수·원본 조회 기능이며, AI 자동 분석과 제보 승인/반려·게시 연동은 별도 구현 사항입니다.

Supabase Authentication에서 Email 공급자와 회원가입을 활성화하고, 일반 회원의 Confirm email을 켜세요. Site URL은 실제 사이트 주소로 설정하고 로컬 개발 주소 `http://127.0.0.1:5173`도 필요한 Redirect URLs에 등록하세요. 확인 메일 인증 후 사이트 로그인 화면에서 이메일과 비밀번호로 로그인합니다. 관리자 계정을 Dashboard에서 직접 만들 때는 Auto confirm user를 선택할 수 있으며, 해당 UID를 `admin_users`에 별도 등록해야 관리자 권한이 생깁니다. Auto confirm 자체는 관리자 권한을 부여하지 않습니다.
